...
def train_model(config: ModelConfig):
    ...
    accum_steps = 4  # Define the number of gradient accumulation steps
    ...
    for epoch in range(initial_epoch, config.num_epochs):
        ...
        for batch_idx, batch in enumerate(batch_iterator):
            ...
            # Update the weights every `accum_steps`
            if (batch_idx + 1) % accum_steps == 0 or (batch_idx + 1) == len(batch_iterator):
                optimizer.step()
