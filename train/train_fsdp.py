...
# Distributed training with FullyShardedDataParallel
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config.num_epochs):
        ...
        if config.global_rank == 0:
            ...
            model.state_dict() # this consolidates the model state
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(), # We don't need to access module here bc we are using fsdp :) 
